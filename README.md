...
I want to thank [Professor Georgios Smaragdakis](https://gsmaragd.github.io/) and [Professor Harm Griffioen](https://harmgriffioen.github.io/) for their support and guidance with my project. They have been an invaluable source of knowledge throughout these 10 weeks. 

For those who want to read my thesis, it can be found [here](https://www.overleaf.com/read/yvczsdjxfygj#94d513).
...
The experiments I ran can be reproduced by building the Docker container and running it. The steps are shown below. Please make sure you cloned the repository beforehand.  

1. Build the Docker container

Firstly, make sure that you are in the *scripts* folder, where the Dockerfile is located. Make sure your docker daemon is also running. Then run: 
...
An example usage of our application could be something along the lines (doing the `Installation` stage):
    ...
    - Running `python check.py` to filter out the offline DNS hosts.
...
This is a fork from CISPA's repository for the [Loopy Attacks code](https://github.com/cispa/loop-DoS). The code in essence is the same, and the methodology is the one Pan et al. created, however I made some small adjustments since the dataset I worked with had much fewer entries. More explanations are provided in the thesis.
